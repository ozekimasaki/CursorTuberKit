...
- **Optimize for continuity.** Prefer prefetching and background generation during current speech so `次ネタ待ち` gaps stay short. Auto-reply generation now pipelines voice synthesis while streaming AI text, so audio segments are ready before playback starts.
...
- Queue-based auto-reply generation (`pumpAutoReplyGenerationQueue`) now streams AI responses and synthesizes voice segments in parallel. The resulting audio blobs are stored in `PreparedAutoReply.audioSegments` so `playPreparedAutoReply` can start playback immediately without waiting for synthesis.
- Comment compaction trigger was lowered from 6 to 3 comments so shorter queues are processed faster.
